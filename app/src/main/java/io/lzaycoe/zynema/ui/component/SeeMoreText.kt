...
import io.lzaycoe.zynema.ui.theme.LinkColor
import io.lzaycoe.zynema.ui.theme.SecondaryFontColor
import io.lzaycoe.zynema.ui.theme.Teal200
...
fun ExpandingText(
    ...
    val textWithLinks = buildAnnotatedString {
        ...
        while (matcher.find()) {
            ...
            val beforeMatch =
                text.substring(startIndex = previousMatchStart, endIndex = matchStart - 2)
            val tagMatch =
                text.substring(
                    startIndex = text.indexOf(char = '>', startIndex = matchStart) + 1,
                    endIndex = text.indexOf(char = '<', startIndex = matchStart + 1),
                )
            append(beforeMatch)
        ...
        if (text.length > matchEnd) {
            append(text.substring(startIndex = matchEnd, endIndex = text.length))
        }
    }
        ...
        when {
            isExpanded -> {
                textWithMoreLess = buildAnnotatedString {
                    ...
                    withStyle(SpanStyle(Teal200)) { append(" See less") }
                    pop()
                }
            }
            ...
            !isExpanded && textLayoutResult.hasVisualOverflow -> {
                val lastCharIndex =
                    textLayoutResult.getLineEnd(visibleLines - 1) // Use visibleLines
                ...
                textWithMoreLess = buildAnnotatedString {
                    append(adjustedText)
                    pushStringAnnotation(tag = "show_more_tag", annotation = "")
                    withStyle(SpanStyle(Teal200)) { append(showMoreString) }
                    pop()
                }
    ...
    SelectionContainer {
        ClickableText(
            ...
            style = TextStyle(color = SecondaryFontColor, fontSize = 15.sp),
            onClick = { offset ->
                textWithMoreLess
                ...
                if (isClickable) {
                    textWithMoreLess
                        .getStringAnnotations(tag = "show_more_tag", start = offset, end = offset)
                        .firstOrNull()
