 ...
 *
 * This property accesses the system's [ConnectivityManager] and checks if the device has an active
 * internet connection.
 *
 * @return [ConnectionState.Available] if there is an active internet connection, otherwise
 ...
 *
 * This function uses the [ConnectivityManager] to check if the currently active network has
 * internet capability.
 *
 ...
 *
 * This function uses a [ConnectivityManager.NetworkCallback] to listen for changes in network
 * connectivity and emits updates via [callbackFlow].
 *
 ...
 *
 * This callback notifies the provided [callback] function of the current [ConnectionState] whenever
 * the network is available or lost.
 *
 * @param callback A function that receives the current [ConnectionState].
 * @return A [ConnectivityManager.NetworkCallback] instance that can be registered with a
 *   [ConnectivityManager].
 */
 ...
 *
 * This function uses [observeConnectivityAsFlow] to observe changes in connectivity and updates the
 * returned [State] value with the latest [ConnectionState].
 *
 * @return A [State] object that holds the latest [ConnectionState] (either Available or
 *   Unavailable).
 */
@Composable
